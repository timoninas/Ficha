//
//  Navigation+View+Extension.swift
//  
//
//  Created by Anton Timonin on 14.10.2022.
//
...
public extension View {
    ...
    func navigation<Destination: View>(isActive: Binding<Bool>,
        ...
        overlay(
            NavigationLink(destination: isActive.wrappedValue ? destination() : nil,
                           isActive: isActive,
                           label: { EmptyView() })
        )
    }
...
public extension NavigationLink {
    ...
    init<T: Identifiable, D: View>(item: Binding<T?>,
        ...
        self.init(destination: item.wrappedValue.map(destination),
                  isActive: isActive,
                  label: label)
    }
